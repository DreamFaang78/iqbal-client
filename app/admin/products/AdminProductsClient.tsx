                  ...
                  <tr key={product.id} className="hover:bg-white/5 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="relative w-10 h-10 rounded bg-[#0E1F12] overflow-hidden border border-white/10 shrink-0">
                          {product.image_url ? (
                            <Image src={product.image_url.split(',')[0]} alt={product.name} fill sizes="40px" className="object-cover" />
                          ) : (
                    ...
                    <div className="flex flex-wrap gap-3">
                      {editProduct.image_url.split(',').filter(Boolean).map((url, index) => (
                        <div key={index} className="w-24 h-24 rounded-xl bg-[#0E1F12] border border-white/10 overflow-hidden relative group">
                          <Image src={url} alt={`Preview ${index + 1}`} fill sizes="96px" className="object-cover" />
                          <button 
                            type="button"
                            onClick={() => {
